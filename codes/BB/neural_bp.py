...
class SyndromeConsistencyLoss(nn.Module):
    ...
    def forward(self, logits_flat, true_syndrome_flat, edge_index_v2c):
        ...
        pred_sign = scatter(sign_edges, check_idx, dim=0, reduce="mul",
                            dim_size=true_syndrome_flat.size(0))

        log_edges = log_abs_val[var_idx]
        log_pred_abs = scatter(log_edges, check_idx, dim=0, reduce="sum",
                               dim_size=true_syndrome_flat.size(0))
...
def _is_oom(exc: BaseException) -> bool:
    ...
    if hasattr(torch, "cuda") and hasattr(torch.cuda, "OutOfMemoryError") and isinstance(exc, torch.cuda.OutOfMemoryError):
        return True
...
def run_training(args):
    ...
    train_loader = PyGDataLoader(OnlineBBPDataset(H, L, probs, args.p, rank=rank),
                                 batch_size=args.batch_size, num_workers=worker_per_gpu, pin_memory=True)
    val_loader = PyGDataLoader(OnlineBBPDataset(H, L, probs, args.p, rank=rank),
                               batch_size=args.batch_size, num_workers=val_workers, pin_memory=True)
    ...
    if args.resume and os.path.exists(args.resume):
        if rank == 0: print(f"--> Transfer weights from: {args.resume}")
    ...
    def lr_lambda(current_step):
        warmup_steps = 100; decay_steps = args.max_steps; min_lr_ratio = 0.05
    ...
    if rank == 0:
        print(f"--> Target Total BS: {args.target_bs} | Micro BS: {args.batch_size} | World Size: {world_size} | ACCUM: {ACCUM}")
    ...
    while update_step < args.max_steps:
        ...
        except StopIteration: iterator = iter(train_loader); data = next(iterator)
        ...
        with my_context:
            ...
            if not torch.isfinite(loss):
                if rank == 0:
                    print(f"[WARN] non-finite loss at update_step={update_step} micro={micro}, skipping this micro-batch", flush=True)
                nan_skips += 1
        ...
        if micro % ACCUM == 0:
            if has_bad_grad(model.parameters()):
                if rank == 0:
                    print(f"[WARN] non-finite grad at update_step={update_step}, skipping", flush=True)
                nan_skips += 1
                optimizer.zero_grad(set_to_none=True)
                continue
            ...
            if update_step % 100 == 0:
                ...
                if rank == 0:
                    ...
                    print(f"[{wall}] Step {update_step:5d} | LR {lr:.2e} | iter={cur_iter} | "
                          ...
                          f"skips={nan_skips}", flush=True)
                    per_log_str = " ".join(
                        f"L{i}={metrics[3 + i].item() * 100:.3f}%"
                        for i in range(metrics.numel() - 3)
                    )
                    print(f"           PER_LOGICAL_ACC step={update_step} {per_log_str}", flush=True)
                    ...
                    if cur_acc > best_acc:
                        ...
                        print(f"  -> new best block {best_acc*100:.3f}% "
                              f"(per-log mean {metrics[1]*100:.3f}%) saved to {best_path}", flush=True)
...
def main_train_neural_bp():
    ...
    parser.add_argument("--num_iter", type=int, default=8,
                        help="Fixed number of Neural-BP iterations.")
    ...
    parser.add_argument("--target_bs", type=int, default=2048,
                        help="Target effective global batch size.")
    parser.add_argument("--num_workers", type=int, default=None,
                        help="DataLoader training workers per rank.")
    parser.add_argument("--val_workers", type=int, default=None,
                        help="DataLoader validation workers per rank.")
...
def main():
    command = "train"
    if len(sys.argv) > 1 and sys.argv[1] in {"train", "generate-dems"}:
